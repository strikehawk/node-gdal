...
/**
 * Gets the bounding box of the dataset in native projection units.
 * 
 * @for gdal.Dataset
 * @method getBoundingBox
 * @return {Integer[]} The bounding box of the Dataset in [minx, miny, maxx, maxy].
 */
gdal.Dataset.prototype.getBoundingBox = function() {
	const cols = this.rasterSize.x;
	const rows = this.rasterSize.y;

	const geotransform = this.geoTransform;
	...
	const width = cols * pixelWidth;
	const height = rows * pixelHeight;

	const bb1 = originX;
	const bb3 = originX + width;
	const bb2 = originY + height;
	const bb4 = originY;
