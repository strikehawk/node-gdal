#ifndef __NODE_GDAL_VRTSOURCEDRASTERBAND_H__
#define __NODE_GDAL_VRTSOURCEDRASTERBAND_H__
...
class VrtSourcedRasterBand: public Nan::ObjectWrap {
public:
    static Nan::Persistent<FunctionTemplate> constructor;
	static void Initialize(Local<Object> target);
	static NAN_METHOD(New);
    static Local<Value> New(VRTSourcedRasterBand *band, VRTDataset *parent);

	// static NAN_METHOD(getMetadataDomainList);
	// static NAN_METHOD(getMetadataItem);
	// static NAN_METHOD(getMetadata);
	// static NAN_METHOD(setMetadata);
	// static NAN_METHOD(setMetadataItem);
	...
	static NAN_METHOD(addSource); //TODO: Uses VrtSimpleSource instead of VrtSource
	static NAN_METHOD(addSimpleSource);
	// static NAN_METHOD(addComplexSource);
	// static NAN_METHOD(addMaskBandSource);
	// static NAN_METHOD(addFuncSource);
	static NAN_METHOD(configureSource);
    ...
    static ObjectCache<VRTSourcedRasterBand, VrtSourcedRasterBand> cache;

    VrtSourcedRasterBand();
	VrtSourcedRasterBand(VRTSourcedRasterBand *band);
    inline bool isAlive(){
		return this_ && ptr_manager.isAlive(uid);
	}
	inline VRTSourcedRasterBand *get() {
		return this_;
	}
	inline VRTDataset *getParent() {
		return parent_ds;
	}
	void dispose();
    long uid;
private:
    ~VrtSourcedRasterBand();
    VRTSourcedRasterBand *this_;
    VRTDataset *parent_ds;
};
