...
Nan::Persistent<FunctionTemplate> VrtSourcedRasterBand::constructor;
ObjectCache<VRTSourcedRasterBand, VrtSourcedRasterBand> VrtSourcedRasterBand::cache;

void VrtSourcedRasterBand::Initialize(Local<Object> target)
{
	Nan::HandleScope scope;

	Local<FunctionTemplate> lcons = Nan::New<FunctionTemplate>(VrtSourcedRasterBand::New);
	// lcons->Inherit(Nan::New(RasterBand::constructor)); 
    lcons->InstanceTemplate()->SetInternalFieldCount(1);
	lcons->SetClassName(Nan::New("VrtSourcedRasterBand").ToLocalChecked());
	...
	Nan::SetPrototypeMethod(lcons, "addSource", addSource);
	Nan::SetPrototypeMethod(lcons, "addSimpleSource", addSimpleSource);
	// Nan::SetPrototypeMethod(lcons, "addComplexSource", addComplexSource);
	// Nan::SetPrototypeMethod(lcons, "addMaskBandSource", addMaskBandSource);
	// Nan::SetPrototypeMethod(lcons, "addFuncSource", addFuncSource);
	Nan::SetPrototypeMethod(lcons, "configureSource", configureSource);
	...
	target->Set(Nan::New("VrtSourcedRasterBand").ToLocalChecked(), lcons->GetFunction());

	constructor.Reset(lcons);
}

VrtSourcedRasterBand::VrtSourcedRasterBand(VRTSourcedRasterBand *band)
...
void VrtSourcedRasterBand::dispose()
{
	if (this_) {
 ...
 *
 * @class gdal.VrtSourcedRasterBand
 */
NAN_METHOD(VrtSourcedRasterBand::New)
{
	Nan::HandleScope scope;
	...
	if (info[0]->IsExternal()) {
		...
		void* ptr = ext->Value();
		VrtSourcedRasterBand *f = static_cast<VrtSourcedRasterBand *>(ptr);
		f->Wrap(info.This());
...
Local<Value> VrtSourcedRasterBand::New(VRTSourcedRasterBand *raw, VRTDataset *raw_parent)
{
	...
	VrtSourcedRasterBand *wrapped = new VrtSourcedRasterBand(raw);

	Local<Value> ext = Nan::New<External>(wrapped);
	Local<Object> obj = Nan::NewInstance(Nan::New(VrtSourcedRasterBand::constructor)->GetFunction(), 1, &ext).ToLocalChecked();
	...
	//add reference to dataset so dataset doesnt get GC'ed while band is alive
		//DONT USE VRTSourcedRasterBand.GetDataset() ... it will return a "fake" dataset for overview bands
		//https://github.com/naturalatlas/node-gdal/blob/master/deps/libgdal/gdal/frmts/gtiff/geotiff.cpp#L84

	Local<Object> ds;
	if (VrtDataset::vrtdataset_cache.has(raw_parent)) {
		ds = VrtDataset::vrtdataset_cache.get(raw_parent);
	} else {
		...
		//ds = VrtDataset::New(raw_parent); //this should never happen
	}

	long parent_uid = Nan::ObjectWrap::Unwrap<VrtDataset>(ds)->uid;
...
NAN_METHOD(VrtSourcedRasterBand::toString)
{
	Nan::HandleScope scope;
	info.GetReturnValue().Set(Nan::New("VrtSourcedRasterBand").ToLocalChecked());
}
...
//  * @method flush
//  */
// NODE_WRAPPED_METHOD(VrtSourcedRasterBand, flush, FlushCache);
...
//  */
// NODE_WRAPPED_METHOD_WITH_RESULT(VrtSourcedRasterBand, getMaskFlags, Integer, GetMaskFlags);
...
//  */
// NODE_WRAPPED_METHOD_WITH_CPLERR_RESULT_1_INTEGER_PARAM(VrtSourcedRasterBand, createMaskBand, CreateMaskBand, "mask flags");
// // TODO: expose GMF constants in API
// // ({{#crossLink "Constants (GMF)"}}see flags{{/crossLink}})
...
//  *
//  * @method getMaskBand
//  * @return {gdal.VrtSourcedRasterBand}
//  */
// NAN_METHOD(VrtSourcedRasterBand::getMaskBand)
// {
// 	Nan::HandleScope scope;

// 	VrtSourcedRasterBand *band = Nan::ObjectWrap::Unwrap<VrtSourcedRasterBand>(info.This());
// 	if (!band->isAlive()) {
// 		Nan::ThrowError("VrtSourcedRasterBand object has already been destroyed");
// 		return;
// 	}
...
//  */
// NAN_METHOD(VrtSourcedRasterBand::fill)
// {
// 	Nan::HandleScope scope;
...
//  */
// NAN_METHOD(VrtSourcedRasterBand::getStatistics)
// {
// 	Nan::HandleScope scope;
...
//  * @return {Object} Statistics containing `"min"`, `"max"`, `"mean"`, `"std_dev"` properties.
//  */
// NAN_METHOD(VrtSourcedRasterBand::computeStatistics)
// {
// 	Nan::HandleScope scope;
// 	double min, max, mean, std_dev;
// 	int approx;
...
//  */
// NAN_METHOD(VrtSourcedRasterBand::setStatistics)
// {
// 	Nan::HandleScope scope;
// 	double min, max, mean, std_dev;
...
//  * @param {string} [domain]
//  */
// NAN_METHOD(VrtSourcedRasterBand::getMetadata)
// {
// 	Nan::HandleScope scope;
...
NAN_METHOD(VrtSourcedRasterBand::addSource)
{
	Nan::HandleScope scope;
	...
	VrtSimpleSource* source;
	NODE_ARG_WRAPPED(0, "source", VrtSimpleSource, source);

	CPLErr err = band->this_->AddSource(source->get());
...
NAN_METHOD(VrtSourcedRasterBand::addSimpleSource)
{
	Nan::HandleScope scope;
	...
	char *resampling;
	double noDataValue;
	NODE_ARG_WRAPPED(0, "source band", RasterBand, srcBand);
	NODE_ARG_DOUBLE(1, "src x off", srcXOff);
	NODE_ARG_DOUBLE(2, "src y off", srcYOff);
	NODE_ARG_DOUBLE(3, "src x size", srcXSize);
	NODE_ARG_DOUBLE(4, "src y size", srcYSize);
	NODE_ARG_DOUBLE(5, "dst x off", dstXOff);
	NODE_ARG_DOUBLE(6, "dst y off", dstYOff);
	NODE_ARG_DOUBLE(7, "dst x size", dstXSize);
	NODE_ARG_DOUBLE(8, "dst y size", dstYSize);
	NODE_ARG_STR(9, "resampling", resampling);
	NODE_ARG_DOUBLE(10, "no data value", noDataValue);

	CPLErr err = band->this_->AddSimpleSource(
		srcBand->get(),
		srcXOff, srcYOff, srcXSize, srcYSize,
		dstXOff, dstYOff, dstXSize, dstYSize,
		resampling, noDataValue);
...
NAN_METHOD(VrtSourcedRasterBand::configureSource)
{
	Nan::HandleScope scope;
	...
	double dstYSize;
	NODE_ARG_WRAPPED(0, "source", VrtSimpleSource, source);
	NODE_ARG_WRAPPED(1, "source band", RasterBand, srcBand);
	NODE_ARG_INT(2, "add as mask band", addAsMaskBand);
	NODE_ARG_DOUBLE(3, "src x off", srcXOff);
	NODE_ARG_DOUBLE(4, "src y off", srcYOff);
	NODE_ARG_DOUBLE(5, "src x size", srcXSize);
	NODE_ARG_DOUBLE(6, "src y size", srcYSize);
	NODE_ARG_DOUBLE(7, "dst x off", dstXOff);
	NODE_ARG_DOUBLE(8, "dst y off", dstYOff);
	NODE_ARG_DOUBLE(9, "dst x size", dstXSize);
	NODE_ARG_DOUBLE(10, "dst y size", dstYSize);

	band->this_->ConfigureSource(
		source->get(),
		srcBand->get(),
		addAsMaskBand,
		srcXOff, srcYOff, srcXSize, srcYSize,
 ...
 * @readOnly
 * @attribute ds
 * @type {gdal.VrtDataset}
 */
NAN_GETTER(VrtSourcedRasterBand::dsGetter)
{
	Nan::HandleScope scope;
 ...
 */
NAN_GETTER(VrtSourcedRasterBand::overviewsGetter)
{
	Nan::HandleScope scope;
 ...
 */
NAN_GETTER(VrtSourcedRasterBand::pixelsGetter)
{
	Nan::HandleScope scope;
 ...
 * @type {Integer|null}
 */
NAN_GETTER(VrtSourcedRasterBand::idGetter)
{
	Nan::HandleScope scope;
	VrtSourcedRasterBand *band = Nan::ObjectWrap::Unwrap<VrtSourcedRasterBand>(info.This());
	if (!band->isAlive()) {
 ...
 * @type {string}
 */
NAN_GETTER(VrtSourcedRasterBand::descriptionGetter) {
	Nan::HandleScope scope;
 ...
 * @type {Object}
 */
NAN_GETTER(VrtSourcedRasterBand::sizeGetter)
{
	Nan::HandleScope scope;
	VrtSourcedRasterBand *band = Nan::ObjectWrap::Unwrap<VrtSourcedRasterBand>(info.This());
	if (!band->isAlive()) {
 ...
 * @type {Object}
 */
NAN_GETTER(VrtSourcedRasterBand::blockSizeGetter)
{
	Nan::HandleScope scope;
	VrtSourcedRasterBand *band = Nan::ObjectWrap::Unwrap<VrtSourcedRasterBand>(info.This());
	if (!band->isAlive()) {
 ...
 * @type {Number}
 */
NAN_GETTER(VrtSourcedRasterBand::minimumGetter)
{
	Nan::HandleScope scope;
	VrtSourcedRasterBand *band = Nan::ObjectWrap::Unwrap<VrtSourcedRasterBand>(info.This());
	if (!band->isAlive()) {
 ...
 * @type {Number}
 */
NAN_GETTER(VrtSourcedRasterBand::maximumGetter)
{
	Nan::HandleScope scope;
	VrtSourcedRasterBand *band = Nan::ObjectWrap::Unwrap<VrtSourcedRasterBand>(info.This());
	if (!band->isAlive()) {
 ...
 * @type {Number}
 */
NAN_GETTER(VrtSourcedRasterBand::offsetGetter)
{
	Nan::HandleScope scope;
	VrtSourcedRasterBand *band = Nan::ObjectWrap::Unwrap<VrtSourcedRasterBand>(info.This());
	if (!band->isAlive()) {
 ...
 * @type {Number}
 */
NAN_GETTER(VrtSourcedRasterBand::scaleGetter)
{
	Nan::HandleScope scope;
	VrtSourcedRasterBand *band = Nan::ObjectWrap::Unwrap<VrtSourcedRasterBand>(info.This());
	if (!band->isAlive()) {
 ...
 * @type {Number|null}
 */
NAN_GETTER(VrtSourcedRasterBand::noDataValueGetter)
{
	Nan::HandleScope scope;
	VrtSourcedRasterBand *band = Nan::ObjectWrap::Unwrap<VrtSourcedRasterBand>(info.This());
	if (!band->isAlive()) {
 ...
 * @type {String}
 */
NAN_GETTER(VrtSourcedRasterBand::unitTypeGetter)
{
	Nan::HandleScope scope;
	VrtSourcedRasterBand *band = Nan::ObjectWrap::Unwrap<VrtSourcedRasterBand>(info.This());
	if (!band->isAlive()) {
 ...
 */
NAN_GETTER(VrtSourcedRasterBand::dataTypeGetter)
{
	Nan::HandleScope scope;
	VrtSourcedRasterBand *band = Nan::ObjectWrap::Unwrap<VrtSourcedRasterBand>(info.This());
	if (!band->isAlive()) {
 ...
 * @type {Boolean}
 */
NAN_GETTER(VrtSourcedRasterBand::readOnlyGetter)
{
	Nan::HandleScope scope;
	VrtSourcedRasterBand *band = Nan::ObjectWrap::Unwrap<VrtSourcedRasterBand>(info.This());
	if (!band->isAlive()) {
 ...
 * @type {Boolean}
 */
NAN_GETTER(VrtSourcedRasterBand::hasArbitraryOverviewsGetter)
{
	Nan::HandleScope scope;
	VrtSourcedRasterBand *band = Nan::ObjectWrap::Unwrap<VrtSourcedRasterBand>(info.This());
	if (!band->isAlive()) {
 ...
 * @type {Array}
 */
NAN_GETTER(VrtSourcedRasterBand::categoryNamesGetter)
{
	Nan::HandleScope scope;
	VrtSourcedRasterBand *band = Nan::ObjectWrap::Unwrap<VrtSourcedRasterBand>(info.This());
	if (!band->isAlive()) {
 ...
 * @type {string}
 */
NAN_GETTER(VrtSourcedRasterBand::colorInterpretationGetter)
{
	Nan::HandleScope scope;
	VrtSourcedRasterBand *band = Nan::ObjectWrap::Unwrap<VrtSourcedRasterBand>(info.This());
	if (!band->isAlive()) {
...
NAN_SETTER(VrtSourcedRasterBand::unitTypeSetter)
{
	Nan::HandleScope scope;
	VrtSourcedRasterBand *band = Nan::ObjectWrap::Unwrap<VrtSourcedRasterBand>(info.This());
	if (!band->isAlive()) {
...
NAN_SETTER(VrtSourcedRasterBand::noDataValueSetter)
{
	Nan::HandleScope scope;
	VrtSourcedRasterBand *band = Nan::ObjectWrap::Unwrap<VrtSourcedRasterBand>(info.This());
	if (!band->isAlive()) {
...
NAN_SETTER(VrtSourcedRasterBand::scaleSetter)
{
	Nan::HandleScope scope;
	VrtSourcedRasterBand *band = Nan::ObjectWrap::Unwrap<VrtSourcedRasterBand>(info.This());
	if (!band->isAlive()) {
...
NAN_SETTER(VrtSourcedRasterBand::offsetSetter)
{
	Nan::HandleScope scope;
	VrtSourcedRasterBand *band = Nan::ObjectWrap::Unwrap<VrtSourcedRasterBand>(info.This());
	if (!band->isAlive()) {
...
NAN_SETTER(VrtSourcedRasterBand::categoryNamesSetter)
{
	Nan::HandleScope scope;
	VrtSourcedRasterBand *band = Nan::ObjectWrap::Unwrap<VrtSourcedRasterBand>(info.This());
	if (!band->isAlive()) {
...
NAN_SETTER(VrtSourcedRasterBand::colorInterpretationSetter)
{
	Nan::HandleScope scope;
	VrtSourcedRasterBand *band = Nan::ObjectWrap::Unwrap<VrtSourcedRasterBand>(info.This());
	if (!band->isAlive()) {
...
NAN_GETTER(VrtSourcedRasterBand::uidGetter)
{
	Nan::HandleScope scope;
	VrtSourcedRasterBand *band = Nan::ObjectWrap::Unwrap<VrtSourcedRasterBand>(info.This());
